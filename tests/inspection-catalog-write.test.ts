...
const ctx: Ctx = { ids: new Map([["brands", new Map()], ["models", new Map()], ["specs", new Map()], ["grades", new Map()]]) };

async function cleanup(target: Target, table: string, code: string) {
  registerCleanup(`delete-catalog-${table}:${target.name}:${code.slice(-8)}`, async () => {
    const tr = await probeRequest(target, { method: "DELETE", path: `/api/catalog/${table}/${code}` });
    if (tr.status !== 200 && tr.status !== 204 && tr.status !== 404) {
      console.warn(`[teardown] delete catalog/${table}/${code} ${target.name} status=${tr.status}`);
    }
  });
}
  ...
  for (const target of targets) {
    it(`${target.name} 创 brand → 200 + 字段齐全`, async () => {
      ...
      const r = await probeRequest(target, {
        ...
        body: { code, name: `brand ${code}`, inspectionObjectCode: SEED_OBJECT_CODE } as Body,
      });
      expect([200, 201], `${target.name} POST brand 期望 200/201 实得 ${r.status} body=${JSON.stringify(r.body).slice(0, 200)}`).toContain(r.status);
...
describe.skipIf(!live)("M96.F02.I03 PUT /api/catalog/brands/{code} 四方比对", () => {
  for (const target of targets) {
    it(`${target.name} 改 name → 200`, async () => {
      ...
      expect([200], `${target.name} PUT brand 期望 200 实得 ${r.status}`).toContain(r.status);
    }, 30_000);
  }
});
...
describe.skipIf(!live)("M96.F02.I04 DELETE /api/catalog/brands/{code} 四方比对", () => {
  it("brand 删除 → 200/204 + 重复删 → 404", async () => {
    for (const target of targets) {
      const code = ctx.ids.get("brands")!.get(target.name);
      if (!code) continue;
      const first = await probeRequest(target, { method: "DELETE", path: `/api/catalog/brands/${code}` });
      expect([200, 204], `${target.name} DELETE brand 期望 200/204 实得 ${first.status}`).toContain(first.status);
      const second = await probeRequest(target, { method: "DELETE", path: `/api/catalog/brands/${code}` });
      expect([404, 204], `${target.name} 重复删 brand 期望 404/204 实得 ${second.status}`).toContain(second.status);
  ...
  for (const target of targets) {
    it(`${target.name} 创 model → 200`, async () => {
      ...
      const r = await probeRequest(target, {
        ...
        body: { code, name: `model ${code}`, inspectionObjectCode: SEED_OBJECT_CODE } as Body,
      });
      expect([200, 201], `${target.name} POST model 期望 200/201 实得 ${r.status}`).toContain(r.status);
...
describe.skipIf(!live)("M96.F02.I07 PUT /api/catalog/models/{code} 四方比对", () => {
  for (const target of targets) {
    it(`${target.name} 改 name → 200`, async () => {
      ...
      expect([200], `${target.name} PUT model 期望 200 实得 ${r.status}`).toContain(r.status);
    }, 30_000);
  }
});
...
describe.skipIf(!live)("M96.F02.I08 DELETE /api/catalog/models/{code} 四方比对", () => {
  it("model 删除 → 200/204 + 重复删 → 404", async () => {
    for (const target of targets) {
      const code = ctx.ids.get("models")!.get(target.name);
      if (!code) continue;
      const first = await probeRequest(target, { method: "DELETE", path: `/api/catalog/models/${code}` });
      expect([200, 204], `${target.name} DELETE model 期望 200/204 实得 ${first.status}`).toContain(first.status);
  ...
  for (const target of targets) {
    it(`${target.name} 创 spec → 200`, async () => {
      ...
      const r = await probeRequest(target, {
        ...
        body: { code, name: `spec ${code}`, inspectionObjectCode: SEED_OBJECT_CODE } as Body,
      });
      expect([200, 201], `${target.name} POST spec 期望 200/201 实得 ${r.status}`).toContain(r.status);
...
describe.skipIf(!live)("M96.F02.I11 PUT /api/catalog/specs/{code} 四方比对", () => {
  for (const target of targets) {
    it(`${target.name} 改 name → 200`, async () => {
      ...
      expect([200], `${target.name} PUT spec 期望 200 实得 ${r.status}`).toContain(r.status);
    }, 30_000);
  }
});
...
describe.skipIf(!live)("M96.F02.I12 DELETE /api/catalog/specs/{code} 四方比对", () => {
  it("spec 删除 → 200/204 + 重复删 → 404", async () => {
    for (const target of targets) {
      const code = ctx.ids.get("specs")!.get(target.name);
      if (!code) continue;
      const first = await probeRequest(target, { method: "DELETE", path: `/api/catalog/specs/${code}` });
      expect([200, 204], `${target.name} DELETE spec 期望 200/204 实得 ${first.status}`).toContain(first.status);
  ...
  for (const target of targets) {
    it(`${target.name} 创 grade → 200`, async () => {
      ...
      const r = await probeRequest(target, {
        ...
        body: { code, name: `grade ${code}`, inspectionObjectCode: SEED_OBJECT_CODE } as Body,
      });
      expect([200, 201], `${target.name} POST grade 期望 200/201 实得 ${r.status}`).toContain(r.status);
...
describe.skipIf(!live)("M96.F02.I15 PUT /api/catalog/grades/{code} 四方比对", () => {
  for (const target of targets) {
    it(`${target.name} 改 name → 200`, async () => {
      ...
      expect([200], `${target.name} PUT grade 期望 200 实得 ${r.status}`).toContain(r.status);
    }, 30_000);
  }
});
...
describe.skipIf(!live)("M96.F02.I16 DELETE /api/catalog/grades/{code} 四方比对", () => {
  it("grade 删除 → 200/204 + 重复删 → 404", async () => {
    for (const target of targets) {
      const code = ctx.ids.get("grades")!.get(target.name);
      if (!code) continue;
      const first = await probeRequest(target, { method: "DELETE", path: `/api/catalog/grades/${code}` });
      expect([200, 204], `${target.name} DELETE grade 期望 200/204 实得 ${first.status}`).toContain(first.status);
