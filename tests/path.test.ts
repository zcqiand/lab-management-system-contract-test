...
describe("pathWithParams 替换", () => {
  it("单个参数替换", () => {
    expect(
      pathWithParams("/tenants/{tenantId}/users", { tenantId: "abc" }),
    ).toBe("/tenants/abc/users");
  });

  it("多个参数按出现顺序逐个替换", () => {
    expect(pathWithParams("/tenants/{t}/users/{u}", { t: "T", u: "U" })).toBe(
      "/tenants/T/users/U",
    );
  });

  it("缺参数立即抛错 —— 不允许沉默 fallback", () => {
    expect(() => pathWithParams("/tenants/{tenantId}/users", {})).toThrow(
      /tenantId/,
    );
  });

  it("多余参数忽略 —— 不想因为多加就炸", () => {
    expect(pathWithParams("/x", { a: "1", b: "2" })).toBe("/x");
  });
  ...
  it("模板不含占位符时不做任何替换", () => {
    expect(pathWithParams("/api/v1/me", { tenantId: "ignored" })).toBe(
      "/api/v1/me",
    );
  });
});
