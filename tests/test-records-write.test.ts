...
describe.skipIf(!live)(
  "M96.F02.I02 POST /api/test-records 四方比对 / M00.F01.I01",
  () => {
    for (const target of targets) {
      it(`${target.name} 创 test-record → 200/201 或 4xx（sample 不存在时）`, async () => {
        ...
        expect(
          [200, 201, 400, 404],
          `${target.name} POST test-record 期望 2xx/4xx 实得 ${r.status}`,
        ).toContain(r.status);
        if (r.status === 200 || r.status === 201) {
          ...
          if (id) {
            ...
            registerCleanup(
              `delete-tr:${target.name}:${id.slice(-8)}`,
              async () => {
                ...
                if (
                  ...
                  console.warn(
                    `[teardown] delete test-record ${target.name} status=${tr.status}`,
                  );
                }
              },
            );
...
describe.skipIf(!live)(
  "M96.F02.I04 PUT /api/test-records/{id} 四方比对 / M01.F04.I01",
  () => {
    for (const target of targets) {
      it(`${target.name} 改 value → 200 或 404（id 不存在时）`, async () => {
        ...
        expect(
          [200, 404],
          `${target.name} PUT test-record 期望 200/404 实得 ${r.status}`,
        ).toContain(r.status);
      }, 30_000);
    }
  },
...
describe.skipIf(!live)(
  "M96.F02.I05 DELETE /api/test-records/{id} 四方比对 / M01.F05.I02",
  () => {
    it("test-record 删除 → 200/204 或 404（id 不存在）", async () => {
      for (const target of targets) {
        ...
        expect(
          [200, 204, 404],
          `${target.name} DELETE test-record 期望 2xx/404 实得 ${r.status}`,
        ).toContain(r.status);
      }
    }, 60_000);
  },
...
describe.skipIf(!live)(
  "M96.F02.I06 PATCH /api/test-records/{id}/verdict 四方比对 / M04.F06.I02",
  () => {
    it("人工改判 verdict → 200 或 404（id 不存在）", async () => {
      for (const target of targets) {
        ...
        expect(
          [200, 404],
          `${target.name} PATCH verdict 期望 200/404 实得 ${r.status}`,
        ).toContain(r.status);
      }
    }, 60_000);
