// M96.F02 — /api/test-records 列表 + 详情 四方比对（Phase 1: 读端点）。
//
// SSOT: test-records.tsp M03.F03 数据录入 —— 检测记录 CRUD + PATCH verdict。
// 本文件只覆盖 GET list + GET {id}；POST/PUT/PATCH/DELETE 是 Phase 2。
...
describe.skipIf(!live)(
  ...
  () => {
    ...
    it("Page<TestRecord> envelope 必填", () => {
      for (const p of probes) {
        const body = p.body as Record<string, unknown>;
        for (const key of ["page", "pageSize", "total", "items"]) {
          expect(
            body[key],
            `${p.target} test-records list 缺 ${key}`,
          ).toBeDefined();
        }
