...
describe.skipIf(!live)(
  "M96.F02.I02 POST /api/inspection/specialties 四方比对 / M00.F01.I01",
  () => {
    beforeAll(() => {
      clearCleanups();
    }, 30_000);
    ...
    for (const target of targets) {
      it(`${target.name} 创 specialty → 200`, async () => {
        ...
        expect(
          [200, 201],
          `${target.name} POST specialty 期望 200/201 实得 ${r.status}`,
        ).toContain(r.status);
        bucket("specialties").set(target.name, code);
        regCleanup(
          target,
          `delete-specialty:${target.name}:${code.slice(-8)}`,
          () =>
            probeRequest(target, {
              method: "DELETE",
...
describe.skipIf(!live)(
  "M96.F02.I03 PUT /api/inspection/specialties/{code} 四方比对 / M01.F04.I02",
  () => {
    for (const target of targets) {
      it(`${target.name} 改 name → 200`, async () => {
        ...
        expect(
          [200],
          `${target.name} PUT specialty 期望 200 实得 ${r.status}`,
        ).toContain(r.status);
      }, 30_000);
    }
  },
...
describe.skipIf(!live)(
  "M96.F02.I04 DELETE /api/inspection/specialties/{code} 四方比对 / M01.F04.I01",
  () => {
    it("specialty 删除 → 200/204", async () => {
      for (const target of targets) {
        ...
        expect(
          [200, 204],
          `${target.name} DELETE specialty 期望 200/204 实得 ${r.status}`,
        ).toContain(r.status);
...
// ── objects ──
describe.skipIf(!live)(
  "M96.F02.I06 POST /api/inspection/objects 四方比对 / M04.F06.I02",
  () => {
    for (const target of targets) {
      it(`${target.name} 创 object → 200`, async () => {
        ...
        expect(
          [200, 201],
          `${target.name} POST object 期望 200/201 实得 ${r.status}`,
        ).toContain(r.status);
        bucket("objects").set(target.name, code);
        regCleanup(
          target,
          `delete-object:${target.name}:${code.slice(-8)}`,
          () =>
            probeRequest(target, {
              method: "DELETE",
...
describe.skipIf(!live)(
  "M96.F02.I07 PUT /api/inspection/objects/{code} 四方比对 / M04.F06.I03",
  () => {
    for (const target of targets) {
      it(`${target.name} 改 name → 200`, async () => {
        ...
        expect(
          [200],
          `${target.name} PUT object 期望 200 实得 ${r.status}`,
        ).toContain(r.status);
      }, 30_000);
    }
  },
...
describe.skipIf(!live)(
  "M96.F02.I08 DELETE /api/inspection/objects/{code} 四方比对 / M04.F06.I04",
  () => {
    it("object 删除 → 200/204", async () => {
      for (const target of targets) {
        ...
        expect(
          [200, 204],
          `${target.name} DELETE object 期望 200/204 实得 ${r.status}`,
        ).toContain(r.status);
...
// ── parameters ──
describe.skipIf(!live)(
  "M96.F02.I10 POST /api/inspection/parameters 四方比对 / M04.F07.I03",
  () => {
    for (const target of targets) {
      it(`${target.name} 创 parameter → 200`, async () => {
        ...
        const r = await probeRequest(target, {
          ...
          body: {
            ...
            name: `prm ${code}`,
            rawName: `raw ${code}`,
            canonicalName: `can ${code}`,
          },
        });
        expect(
          [200, 201],
          `${target.name} POST parameter 期望 200/201 实得 ${r.status}`,
        ).toContain(r.status);
        bucket("parameters").set(target.name, code);
        regCleanup(
          target,
          `delete-parameter:${target.name}:${code.slice(-8)}`,
          () =>
            probeRequest(target, {
              method: "DELETE",
...
describe.skipIf(!live)(
  "M96.F02.I11 PUT /api/inspection/parameters/{code} 四方比对 / M04.F07.I04",
  () => {
    for (const target of targets) {
      it(`${target.name} 改 name → 200`, async () => {
        ...
        expect(
          [200],
          `${target.name} PUT parameter 期望 200 实得 ${r.status}`,
        ).toContain(r.status);
      }, 30_000);
    }
  },
...
describe.skipIf(!live)(
  "M96.F02.I12 DELETE /api/inspection/parameters/{code} 四方比对 / M04.F08.I02",
  () => {
    it("parameter 删除 → 200/204", async () => {
      for (const target of targets) {
        ...
        expect(
          [200, 204],
          `${target.name} DELETE parameter 期望 200/204 实得 ${r.status}`,
        ).toContain(r.status);
...
// ── standards ──
describe.skipIf(!live)(
  "M96.F02.I14 POST /api/inspection/standards 四方比对 / M04.F08.I04",
  () => {
    for (const target of targets) {
      it(`${target.name} 创 standard → 200`, async () => {
        ...
        expect(
          [200, 201],
          `${target.name} POST standard 期望 200/201 实得 ${r.status}`,
        ).toContain(r.status);
        bucket("standards").set(target.name, code);
        regCleanup(
          target,
          `delete-standard:${target.name}:${code.slice(-8)}`,
          () =>
            probeRequest(target, {
              method: "DELETE",
...
describe.skipIf(!live)(
  "M96.F02.I15 PUT /api/inspection/standards/{code} 四方比对 / M06.F02.I02",
  () => {
    for (const target of targets) {
      it(`${target.name} 改 name → 200`, async () => {
        ...
        expect(
          [200],
          `${target.name} PUT standard 期望 200 实得 ${r.status}`,
        ).toContain(r.status);
      }, 30_000);
    }
  },
...
describe.skipIf(!live)(
  "M96.F02.I16 DELETE /api/inspection/standards/{code} 四方比对 / M06.F02.I03",
  () => {
    it("standard 删除 → 200/204", async () => {
      for (const target of targets) {
        ...
        expect(
          [200, 204],
          `${target.name} DELETE standard 期望 200/204 实得 ${r.status}`,
        ).toContain(r.status);
...
describe.skipIf(!live)(
  "M96.F02.I21 POST /api/inspection/links/specialty-object 四方比对 / M06.F02.I04",
  () => {
    for (const target of targets) {
      it(`${target.name} link(SP01, OBJ-SP01-P1) → 200/204`, async () => {
        ...
        expect(
          [200, 201, 204],
          `${target.name} link 期望 2xx 实得 ${r.status}`,
        ).toContain(r.status);
      }, 30_000);
    }
  },
...
describe.skipIf(!live)(
  "M96.F02.I22 DELETE /api/inspection/links/specialty-object 四方比对 / M06.F08.I06",
  () => {
    for (const target of targets) {
      it(`${target.name} unlink(SP01, OBJ-SP01-P1) → 200/204`, async () => {
        ...
        expect(
          [200, 201, 204],
          `${target.name} unlink 期望 2xx 实得 ${r.status}`,
        ).toContain(r.status);
        ...
        expect(
          [200, 201, 204],
          `${target.name} 重复 unlink 期望幂等 2xx 实得 ${r2.status}`,
        ).toContain(r2.status);
      }, 90_000);
    }
  },
...
describe.skipIf(!live)(
  "M96.F02.I24 POST /api/inspection/links/object-parameter 四方比对 / M06.F07.I04",
  () => {
    for (const target of targets) {
...
describe.skipIf(!live)(
  "M96.F02.I25 DELETE /api/inspection/links/object-parameter 四方比对 / M06.F07.I05",
  () => {
    for (const target of targets) {
...
describe.skipIf(!live)(
  "M96.F02.I27 POST /api/inspection/links/object-standard 四方比对 / M06.F05.I03",
  () => {
    for (const target of targets) {
      it(`${target.name} link(OBJ, STD, role=judgment) → 200/204`, async () => {
        const r = await probeRequest(target, {
          ...
          body: {
            inspectionObjectCode: SEED_OBJECT,
            inspectionStandardCode: SEED_STANDARD,
            role: "JUDGMENT",
          },
        });
        expect(
...
describe.skipIf(!live)(
  "M96.F02.I28 DELETE /api/inspection/links/object-standard 四方比对 / M06.F05.I04",
  () => {
    for (const target of targets) {
...
describe.skipIf(!live)(
  "M96.F02.I30 POST /api/inspection/links/standard-parameter 四方比对 / M06.F05.I05",
  () => {
    for (const target of targets) {
...
describe.skipIf(!live)(
  "M96.F02.I31 DELETE /api/inspection/links/standard-parameter 四方比对 / M03.F03.I08",
  () => {
    for (const target of targets) {
