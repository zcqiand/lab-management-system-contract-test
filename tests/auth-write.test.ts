...
describe.skipIf(!live)(
  "M96.F02.I02 POST /api/auth/refresh 四方比对 / M00.F01.I01",
  () => {
    for (const target of targets) {
      it(`${target.name} refresh(有效 token) → 200 + 新 token`, async () => {
        ...
        expect(
          [200, 400, 401],
          `${target.name} refresh 期望 200/4xx 实得 ${r.status}`,
        ).toContain(r.status);
      }, 30_000);
    }
  },
...
describe.skipIf(!live)(
  "M96.F02.I05 POST /api/auth/logout 四方比对 / M01.F05.I02",
  () => {
    for (const target of targets) {
      it(`${target.name} logout → 200/204`, async () => {
        ...
        expect(
          [200, 204],
          `${target.name} logout 期望 200/204 实得 ${r.status}`,
        ).toContain(r.status);
      }, 30_000);
    }
  },
...
describe.skipIf(!live)(
  "M96.F02.I03 POST /api/auth/switch-tenant 四方比对 / M01.F04.I02",
  () => {
    for (const target of targets) {
      it(`${target.name} switch-tenant(有效租户) → 200 + 真 JWT token`, async () => {
        ...
        expect(me.status, `${target.name} /me 期望 200 实得 ${me.status}`).toBe(
          200,
        );
        ...
        expect(
          tid,
          `${target.name} /me 必须返回可切换租户（currentTenantId 或 tenants[0]）`,
        ).toBeTruthy();
        const r = await probeRequest(target, {
          method: "POST",
        ...
        expect(
          r.status,
          `${target.name} switch-tenant(有效) 期望 200 实得 ${r.status}`,
        ).toBe(200);
      ...
      it(`${target.name} switch-tenant → 200 或 4xx（tenant 不存在）`, async () => {
        ...
        expect(
          [200, 400, 404],
          `${target.name} switch-tenant 期望 200/4xx 实得 ${r.status}`,
        ).toContain(r.status);
      }, 30_000);
    }
  },
