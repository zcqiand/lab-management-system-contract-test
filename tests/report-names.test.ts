...
describe.skipIf(!live)(
  `M96.F02.I06 GET ${PATH_LINK_OBJECT} 四方比对 / M04.F06.I02`,
  () => {
    let probes: Probe[];

    beforeAll(async (ctx) => {
      probes = await withLiveExecSuite(ctx.name, () =>
        probeAll(targets, PATH_LINK_OBJECT),
      );
    }, 60_000);
...
describe.skipIf(!live)(
  `M96.F02.I07 GET ${PATH_LINK_STANDARD} 四方比对 / M04.F06.I03`,
  () => {
    let probes: Probe[];

    beforeAll(async (ctx) => {
      probes = await withLiveExecSuite(ctx.name, () =>
        probeAll(targets, PATH_LINK_STANDARD),
      );
    }, 60_000);
...
describe.skipIf(!live)(
  `M96.F02.I08 GET ${PATH_LINK_PARAMETER} 四方比对 / M04.F06.I04`,
  () => {
    let probes: Probe[];

    beforeAll(async (ctx) => {
      probes = await withLiveExecSuite(ctx.name, () =>
        probeAll(targets, PATH_LINK_PARAMETER),
      );
    }, 60_000);
