...
describe.skipIf(!live)(
  "M96.F02.I02 POST /api/receipts 四方比对 / M00.F01.I01",
  () => {
    for (const target of targets) {
      it(`${target.name} 创 receipt → 200/201 或 4xx（contract 不存在）`, async () => {
        ...
        expect(
          [200, 201, 400, 404],
          `${target.name} POST receipt 期望 2xx/4xx 实得 ${r.status}`,
        ).toContain(r.status);
        if (r.status === 200 || r.status === 201) {
          ...
          if (id) {
            ...
            registerCleanup(
              `delete-receipt:${target.name}:${id.slice(-8)}`,
              async () => {
                ...
                if (
                  ...
                  console.warn(
                    `[teardown] delete receipt ${target.name} status=${tr.status}`,
                  );
                }
              },
            );
...
describe.skipIf(!live)(
  "M96.F02.I03 PUT /api/receipts/{id} 四方比对 / M01.F04.I02",
  () => {
    for (const target of targets) {
      it(`${target.name} 改 clientName → 200 或 404`, async () => {
        ...
        expect(
          [200, 404],
          `${target.name} PUT receipt 期望 200/404 实得 ${r.status}`,
        ).toContain(r.status);
      }, 30_000);
    }
  },
...
describe.skipIf(!live)(
  "M96.F02.I04 DELETE /api/receipts/{id} 四方比对 / M01.F04.I01",
  () => {
    it("receipt 删除 → 200/204 或 404（id 不存在）", async () => {
      for (const target of targets) {
        ...
        expect(
          [200, 204, 404],
          `${target.name} DELETE receipt 期望 2xx/404 实得 ${r.status}`,
        ).toContain(r.status);
      }
    }, 60_000);
  },
...
describe.skipIf(!live)(
  "M96.F02.I05 PUT /api/receipts/{id}/task 四方比对 / M01.F05.I02",
  () => {
    it("任务分配 → 200 或 404（id 不存在）", async () => {
      for (const target of targets) {
        ...
        expect(
          [200, 404],
          `${target.name} PUT task 期望 200/404 实得 ${r.status}`,
        ).toContain(r.status);
      }
    }, 60_000);
