...
describe.skipIf(!live)(
  `M96.F02.I02 POST ${PATH_LIST} 四方比对 / M00.F01.I01`,
  () => {
    beforeAll(() => {
      clearCleanups();
      ctx.ids.clear();
    ...
    for (const target of targets) {
      it(`${target.name} 创 contract 返回 200/201 + 字段齐全`, async () => {
        ...
        expect(
          [200, 201],
          `${target.name} POST 期望 200/201 实得 ${r.status} body=${JSON.stringify(r.body).slice(0, 200)}`,
        ).toContain(r.status);
        ...
        registerCleanup(`delete-contract:${target.name}`, async () => {
          ...
          if (tr.status !== 200 && tr.status !== 204 && tr.status !== 404) {
            console.warn(
              `[teardown] delete contract ${target.name} status=${tr.status}`,
            );
          }
        });
      }, 30_000);
    ...
    it("normalize 后成功响应字段一致（除唯一值/时间戳）", async () => {
      ...
      for (const t of targets) {
        ...
        registerCleanup(
          `delete-shape-contract:${t.name}:${shapeId.slice(-8)}`,
          async () => {
            const tr = await probeRequest(t, {
              method: "DELETE",
              path: `${PATH_LIST}/${shapeId}`,
            });
            if (tr.status !== 200 && tr.status !== 204 && tr.status !== 404) {
              console.warn(
                `[teardown] shape delete ${t.name} status=${tr.status}`,
              );
            }
          },
        );
...
describe.skipIf(!live)(
  `M96.F02.I04 PUT /api/contracts/{id} 四方比对 / M01.F04.I01`,
  () => {
    for (const target of targets) {
      it(`${target.name} 改 projectName → 200`, async () => {
        const id = ctx.ids.get(target.name);
        if (!id)
          throw new Error(`${target.name} I02 未创建 contract，跳过 I04`);
        const r = await probeRequest(target, {
          method: "PUT",
          path: `${PATH_LIST}/${id}`,
        ...
        expect([200], `${target.name} PUT 期望 200 实得 ${r.status}`).toContain(
          r.status,
        );
      }, 30_000);
    }
...
describe.skipIf(!live)(
  `M96.F02.I05 DELETE /api/contracts/{id} 四方比对 / M01.F05.I02`,
  () => {
    it("I02 的 contract 删除 → 200/204 + 重复删 → 404（幂等）", async () => {
      for (const target of targets) {
        const id = ctx.ids.get(target.name);
        if (!id)
          throw new Error(`${target.name} I02 未创建 contract，跳过 I05`);
        ...
        expect(
          [404, 204],
          `${target.name} 重复删期望 404/204 实得 ${second.status}`,
    ...
    afterAll(async () => {
      ...
      for (const [tname, id] of ctx.ids) {
        ...
        if (r.status !== 200 && r.status !== 204 && r.status !== 404) {
          console.warn(
            `[teardown] final delete contract ${tname} status=${r.status}`,
          );
        }
      }
    }, 60_000);
