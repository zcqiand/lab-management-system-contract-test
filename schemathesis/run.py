#!/usr/bin/env python3
"""Schemathesis 契约模糊测试入口（TSP SSOT 清理 Phase E1，2026-09-17）。
...
只跑 GET（--include-method GET）：写方法随机体会污染 lab_dev 共库且无法
teardown（设计文档 §E2 Phase 1 硬边界）。请求超时 60s：nextjs dev 单次
login 实测 7.5-8s（src/http.ts 同款水位）。
"""
